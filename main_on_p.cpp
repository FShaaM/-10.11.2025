...
struct IntMatrix
{
  ...
  int Add_Fill_Rows_Cols_0(size_t num_rows, size_t num_cols);
};
...
int main(int argc, const char** argv)
{
  try
  {
    ...
    while (std::cin >> command && !std::cin.eof())
    {
      ...
      {
        if (mat.Add_Fill_Rows_Cols_0(FirComm, SecComm) == 1)
          return 3;
      }
      else
      {
...
int IntMatrix::Add_Fill_Rows_Cols_0(size_t num_rows, size_t num_cols)
{
  try {
    if (Add_Fill_Rows(num_rows, 0) == 1)
      return 1;
    if (Add_Fill_Cols(num_cols, 0) == 1)
      return 1;
    return 0;
  }
  catch (const std::bad_alloc&) {
